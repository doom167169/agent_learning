"""
完整 RAG Agent 流程
查询改写 → 稠密检索 + 稀疏检索 → RRF 融合 → Cross-Encoder 精排 → LLM 生成
"""
...
rewrite_model = init_chat_model("deepseek-v4-flash")          # 查询改写(便宜小模型)
...
def load_and_split(md_path: str) -> List[Document]:
    ...
    for i, doc in enumerate(chunks):
        ...
        if "h3" in doc.metadata:
            doc.page_content = f"### {doc.metadata['h3']}\n{doc.page_content}"
    return chunks
...
# 3. 稀疏检索引擎 (BM25)
# ============================================================
def build_bm25_index(docs: List[Document], index_path: str = "./db/my_index.bm25"):
...
def comprehensive_search(query: str, top_k: int = 3) -> str:
    ...
    dense_docs = [{"id": d.id, "content": d.page_content} for d, _ in dense_results]
